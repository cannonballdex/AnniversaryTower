...
actions.TEXT_BASE_HEIGHT = ImGui.GetTextLineHeightWithSpacing();
...
function actions.text_colored(style, text)
    ...
    elseif (style == TextStyle.Yellow) then
        ImGui.TextColored(0.000, 1.00, 1.00, 1,text)
    else
        ImGui.Text(text)
    end
end
...
function actions.add_action_button(verbiage, actionName, tooltip, actionParameter)
    if ImGui.Button(verbiage) then engine.SetAction(actionName, actionParameter)
    end
    if (tooltip ~= nil) then
        actions.add_tooltip(tooltip)
    end
...
function actions.add_icon_button(icon, verbiage, action, tooltip)
    local display = string.format('%s %s', icon, verbiage)
    if ImGui.Button(display) then action()
    end
    if (tooltip ~= nil) then
        actions.add_tooltip(tooltip)
    end
...
function actions.add_icon_action_button(icon, verbiage, actionName, tooltip, actionParameter)
    local display = string.format('%s %s', icon, verbiage)
    if ImGui.Button(display) then engine.SetAction(actionName, actionParameter)
    end
    if (tooltip ~= nil) then
        actions.add_tooltip(tooltip)
    end
